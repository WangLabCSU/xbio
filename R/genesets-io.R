...
read_gmt <- function(path, ...) {
    ...
    terms <- vapply(data, .subset, 1L, character(1L), USE.NAMES = FALSE)
    ...
    new_genesets(genesets, terms = terms, descriptions = descriptions)
}
...
read_gmx <- function(path, ...) {
    utils::read.table(path, header = FALSE, sep = "\t")
}
